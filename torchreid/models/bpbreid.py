...
def flatten_outputs(
    ...
    B = bn_foreground_embeddings.size(0)  # batch size (=1)

    # 1. foreground embedding: [1, 1920]
    fg_embed = bn_foreground_embeddings

    # 2. parts embeddings: [1, 5, 1920] -> [1, 5*1920]
    parts_embed_flat = parts_embeddings.reshape(B, -1)

    # 3. foreground visibility: scalar / [1] -> [1, 1]
    fg_vis = foreground_visibility.reshape(B, 1)

    # 4. parts visibility: [1, 5]
    # parts_vis = parts_visibility.reshape(B, -1)
    parts_vis = parts_visibility

    # 5. concat -> [1, 11526]
    flat_output = torch.cat(
        [fg_embed, parts_embed_flat, fg_vis, parts_vis],
        dim=1
    )
...
class BPBreID(nn.Module):
    ...
    def __init__(self, num_classes, pretrained, loss, model_cfg, horizontal_stripes=False, struct_mode=0, **kwargs):
        ...
        # Init pooling layers
        self.global_pooling_head = nn.AdaptiveAvgPool2d(1) # 0130 changed
    ...
    #
    #         # p = torch.zeros(*pixels_parts_predictions_org.shape, self.parts_num + 1, dtype=pixels_parts_predictions_org.dtype)
    #         # p.scatter_(3, pixels_parts_predictions_org.unsqueeze(3), 1)
    #         # p.to(torch.int32)
    #         # p = p.permute(0, 3, 1, 2)
    #         # p = p.amax(dim=(2, 3))
    #
    #         p = F.one_hot(
    #             pixels_parts_predictions_org, num_classes=self.parts_num + 1
    #         ).permute(0, 3, 1, 2).to(torch.float32)  # [N, K+1, Hf, Wf], float32
    #
    #         # 2.  [N, K+1, Hf*Wf]
    #         p = p.flatten(2)
    #         # 3. [N, K+1]
    #         p = torch.amax(p, dim=2)  # [N, K+1]
    #
    #         if self.structure_type_for_deployment == 0:
    #             parts_visibility = p.to(torch.bool)
    #         else:
    #             parts_visibility = p
    #
    #     else:
    ...
    #     if self.structure_type_for_deployment == 5:
    #         return flatten_outputs(bn_foreground_embeddings, parts_embeddings, foreground_visibility, parts_visibility)
    #     if self.structure_type_for_deployment == 6:
    #         return bn_foreground_embeddings, parts_embeddings, foreground_visibility, parts_visibility
    #
    ...
    def forward(self, images, external_parts_masks=None):
        ...
        """
        pixels_parts_predictions_org = pixels_parts_probabilities.argmax(dim=1)  # [N, Hf, Wf]
        pixels_parts_predictions = F.one_hot(pixels_parts_predictions_org, 6)  # [N, Hf, Wf, K+1]  int64[1,96,32,OneHotvp_dim_3]
        pixels_parts_predictions_one_hot = pixels_parts_predictions.permute(0, 3, 1, 2)  # [N, K+1, Hf, Wf]
        pixels_parts_predictions_one_hot = pixels_parts_predictions_one_hot.reshape(pixels_parts_predictions_one_hot.shape[0], 6, pixels_parts_predictions_one_hot.shape[2], pixels_parts_predictions_one_hot.shape[3])
        parts_visibility = pixels_parts_predictions_one_hot.amax(dim=(2, 3))  # [N, K+1]
        """

        parts_visibility = pixels_parts_probabilities.amax(dim=(2, 3))  # [N, K+1]

        vf = parts_visibility.amax(dim=1)  # [N]
        vp = parts_visibility[:, 1:]  # [N, K]

        foreground_embeddings = self.foreground_attention_pooling_head(spatial_features,
                                                                       foreground_masks.unsqueeze(1)).flatten(1,
                                                                                                              2)  # [N, D]

        fp = self.parts_attention_pooling_head(spatial_features, parts_masks)  # [N, K, D]
        bnff, foreground_cls_score = self.foreground_identity_classifier(foreground_embeddings)
        # output = flatten_outputs(bnff, fp, vf, vp)
        return bnff, fp, vf, vp
...
class GlobalMaskWeightedPoolingHead(nn.Module):
    ...
    def forward(self, features, part_masks):
        ...
        #### parts_features = parts_features.reshape(N, M, -1)
...
class GlobalMaxPoolingHead(GlobalMaskWeightedPoolingHead):
    def __init__(self, depth, normalization='identity'):
        super().__init__(depth, normalization)
...
class GlobalAveragePoolingHead(GlobalMaskWeightedPoolingHead):
    ...
    # global_pooling = nn.AdaptiveAvgPool2d((1, 1)) # changed 0130
